  ...
  return (
    <div className="p-3 md:p-4 justify-between border-b w-full flex items-center gap-3">
      <div className="flex flex-row items-center gap-3 min-w-0">
        <Button
          className="ps-4 pe-4 bg-red-800 hover:bg-red-600 cursor-pointer shrink-0"
      ...
      <div className="flex gap-2 shrink-0">
        <div className="flex flex-col">
          ...
          <p className="font-bold text-base md:text-xl">
            {product.value === 0 ? "" : (product.value * 1.95583).toFixed(2)}{" "}
          </p>
        </div>
          <div>

          <select className="font-bold text-base md:text-xl" value={product.unit} onChange={(e) => handleChange("unit", e.target.value)}>
            <option value="euro-br">€ / бр</option>
            <option value="euro-kg">€ / кг</option>
          </select>
            <p className="font-bold text-base md:text-xl">{product.unit === "euro-br" ? "лв. / бр." : "лв. / кг."}</p>
          </div>
      </div>
    </div>
  );
